...
7. Delete protection boundaries
   ...
   - ``on_delete`` on these tables expresses containment, not protection. It governs deletion of the row a foreign key points *at*, never the row holding it, and a row whose referent is gone is meaningless. So ``CompetencyCriteriaGroup.parent``, ``.tag`` and ``.course`` cascade, as do ``CompetencyCriterion.group`` and ``.object_tag``, and ``CompetencyRuleProfile.course`` and ``.competency_taxonomy``. Deleting a competency (a ``Tag``) therefore removes its whole criteria tree, and deleting a course run removes both the trees and the profile scoped to that run.
   - The cascading links are load-bearing rather than a relaxation, for two reasons. First, Django's collector finds referencing rows by querying the database, not by consulting the set it has already decided to delete, so a parent and child reached in the same batch would still trip ``PROTECT`` and abort the walk partway down. Second, those same cascading edges are how a delete reaches the ``PROTECT`` on the learner status tables, which is where this decision is actually enforced.
   - ``CompetencyCriterion.rule_profile`` is ``RESTRICT``, not ``PROTECT``, and that difference is what lets the two halves of the bullet above this list coexist. Both values refuse a direct ``CompetencyRuleProfile`` delete while any criterion is assigned to the profile, which is what holds "a profile is never hard-deleted by a direct delete" at the ORM layer. They differ once the profile is deleted as part of a larger operation: ``PROTECT`` raises for any referencing row it finds in the database, including a criterion that the same operation was already about to delete, whereas ``RESTRICT`` raises only for a referencing row that is not itself being deleted in that same operation. Deleting a ``CompetencyTaxonomy`` therefore succeeds, because the collector reaches the taxonomy-scoped profile through ``CompetencyRuleProfile.competency_taxonomy`` and reaches every criterion through the tag chain, leaving nothing to restrict it.
   - ``CompetencyRuleProfile.organization`` is ``PROTECT``. An ``Organization`` is not a competency definition record, and ``edx-organizations`` retires an organization by clearing its ``active`` flag rather than deleting the row, so a profile refusing such a delete is the safe outcome rather than an obstruction.
   - Residual case, recorded rather than fixed. A criterion's profile assignment is independent of ``CompetencyCriteriaGroup.course_id`` (Decision 4), so a criterion in a tree with no course scope may still be assigned a course-scoped profile. Deleting that course run collects the profile but not that criterion, and ``RESTRICT`` raises ``RestrictedError``. That failure is correct rather than spurious, since the criterion really would otherwise be left pointing at a deleted profile, and it is unreachable until course-scoped profiles can be authored. The fix at that point is a fifth reassignment event on Decision 4: when a profile's scope owner is being deleted, reassign every criterion off that profile before the cascade proceeds.
   - The learner status tables (Decision 6) carry the values this decision is ultimately enforced by. ``StudentCompetencyCriteriaStatus.criterion``, ``StudentCompetencyCriteriaGroupStatus.group`` and ``StudentCompetencyStatus.tag`` are ``PROTECT``, so a delete that reaches a node some learner holds status against is refused at the database layer whichever path it arrived by. Each table's ``user`` foreign key is ``CASCADE``, because a learner's status is a derived fact about that learner and ``PROTECT`` there would let this library veto ``User.delete()`` platform-wide, from code that has no reason to know these rows exist. Each table's ``status`` foreign key is ``PROTECT``, because the lookup table it points at is seeded, system-owned data. These three node ``PROTECT`` values are deliberately stricter than the predicate in the third bullet of this list, which names only ``StudentCompetencyCriteriaStatus``: the database draws no distinction between the leaf table and the two roll-ups, so a roll-up row with no leaf beneath it would also block a delete. That should not occur, and failing closed is the right default for a backstop.
   - Foreign keys into tables this decision does not own keep whatever behavior those tables already define. They are listed here only so that the whole delete path is legible in one place: ``Tag.taxonomy`` and ``Tag.parent`` cascade, ``ObjectTag.taxonomy`` and ``ObjectTag.tag`` are ``SET_NULL`` so an object tag outlives the tag whose value it cached, ``CourseRun.catalog_course`` and ``CatalogCourse.org`` are ``PROTECT``, and ``CompetencyTaxonomy``'s link to ``Taxonomy`` is Django's multi-table-inheritance parent link, which always cascades and cannot be configured.
   - ``CompetencyMasteryStatuses`` has no delete protection of its own. The three ``status`` foreign keys make one of its rows undeletable while any learner status row references it, but a row nothing references can still be deleted. "Seeded by migration and never deleted" is a convention, backed by a read-only Django admin rather than by a database constraint, because SQL offers no way to forbid deletion of a row nothing references short of a trigger this project does not use.
...
* For issue #641: ``scope_code`` on ``CompetencyRuleProfile`` is now a plain column recomputed in
  the model's ``save()``, null while a profile is archived, rather than a never-null
  database-generated column. Nulling it while archived is what frees an archived profile's scope
  for a replacement, without the conditional unique index MySQL does not support. The scope
  columns themselves are never cleared, so an archived profile stays restorable.
* For issue #641: Decision 7 now states the ``on_delete`` value of every foreign key on the delete
  path, rather than leaving them to be inferred from the archive-versus-hard-delete rule. The
  criteria tree cascades, ``CompetencyCriterion.rule_profile`` is ``RESTRICT`` so that a scope
  owner's deletion can carry a profile away while a direct profile delete is still refused,
  ``CompetencyRuleProfile.organization`` is ``PROTECT``, and the learner status tables protect the
  node they track, cascade from the learner, and protect the status lookup. The taxonomy-delete
  failure that ``PROTECT`` on ``rule_profile`` would have caused is gone; a narrower course-scoped
  case remains and is recorded.
